...
class PetClassifierCNN(nn.Module):
    ...
    def __init__(self):
        ...
        # DONE: 3 in_channels, 16 out_channels, 3x3 kernel, padding 1
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()
        # DONE: MaxPool 2x2 kernel, stride 2
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # 2. Second Convolutional Block
        # DONE: 16 in_channels, 32 out_channels, 3x3 kernel, padding 1
        ...
        # DONE: Linear layer from flattened_size to 2 output classes
        self.fc = nn.Linear(flattened_size, 2)

    def forward(self, x):
        # DONE: Route x through conv1, relu1, and pool1
        x = self.conv1(x)
        x = self.relu1(x)
        x = self.pool1(x)

        # DONE: Route through conv2, relu2, and pool2
        x = self.conv2(x)
        x = self.relu2(x)
        x = self.pool2(x)
        ...
        # DONE: Route through the final fully connected (fc) layer
        return self.fc(x)
